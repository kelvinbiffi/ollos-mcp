...
async function downloadDirect(url: URL, config: OllosConfig, cache: Cache, signal?: AbortSignal): Promise<string> {
  ...
  const existing = fs.readdirSync(dir).find((f) => f.startsWith('media'))
...
export async function resolveSource(input: string, config: OllosConfig, opts: ResolveOptions = {}): Promise<ResolvedSource> {
  ...
  if (info.durationSec > config.limits.maxDurationSec) {
    throw new OllosError('DURATION_EXCEEDED', `media is ${Math.round(info.durationSec / 60)} min, limit is ${Math.round(config.limits.maxDurationSec / 60)} min`, { hint: 'Use from/to to analyse a window, or raise OLLOS_MAX_DURATION_SEC.' })
  }
